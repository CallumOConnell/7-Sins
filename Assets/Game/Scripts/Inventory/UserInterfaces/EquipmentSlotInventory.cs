...
{
    public class EquipmentSlotInventory : MonoBehaviour
    {
        [SerializeField]
        ...
        private void Start()
        {
            ...
            inventory.OnItemAdded += (item) => EquipmentManager.Instance.EquipArmour(item as Armour);

            // Item has been taken out of slot and put into an inventory slot so unequip the item
            inventory.OnItemRemoved += (item) => EquipmentManager.Instance.UnequipArmour((int)_equipmentSlot);
        }
    }
}