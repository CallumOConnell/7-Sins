...
namespace Sins.Inventory
{
    public class InventoryRenderer : MonoBehaviour
    {
        ...
        private void ReRenderGrid()
        {
            ...
            var containerSize = new Vector2(_cellSize.x * _inventory.Width, _cellSize.y * _inventory.Height);
            ...
            switch (_renderMode)
            {
                ...
                default:
                {
                    var topLeftCorner = new Vector3(-containerSize.x / 2, -containerSize.y / 2, 0);

                    var halfCellSize = new Vector3(_cellSize.x / 2, _cellSize.y / 2, 0);

                    _grids = new Image[_inventory.Width * _inventory.Height];

                    var counter = 0;

                    for (var y = 0; y < _inventory.Height; y++)
                    {
                        for (var x = 0; x < _inventory.Width; x++)
                        {
                            grid = CreateImage(_cellSpriteEmpty, true);
        ...
        {
            var image = CreateImage(item.Sprite, false);
            ...
            var test = item as ItemDefinition;
        ...
        {
            var x = (-(_inventory.Width * 0.5f) + item.Position.x + item.Width * 0.5f) * _cellSize.x;
            var y = (-(_inventory.Height * 0.5f) + item.Position.y + item.Height * 0.5f) * _cellSize.y;
